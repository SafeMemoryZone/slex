...
static int slex_parse_num(SlexContext *ctx) {
  ...
  int len = end - ctx->parse_point;
  unsigned long long num = 0;

  for(char *it = ctx->parse_point; it < end; it++) {
    if(slex_add_overflows_u64((*it - '0') * fact, num)) {
      ctx->parse_point = it;
      return slex_return_err(ctx);
    }
    num += (*it - '0') * fact;
...
static int slex_parse_int_lit(SlexContext *ctx) {
  ...
  if(ctx->parse_point[1] == 'x' || ctx->parse_point[1] == 'X') {
    ctx->parse_point += 2; // consume 0 and x
  ...
  if(slex_is_numeric(ctx->parse_point[1])) {
    ...
    for(char *it = ctx->parse_point; it < end; it++) {
      if(slex_add_overflows_u64((*it - '0') * fact, oct)) {
        ctx->parse_point = it;
        return slex_return_err(ctx);
      }
      oct += (*it - '0') * fact;
...
static int slex_parse_esc_seq(SlexContext *ctx) {
  ...
  if(slex_is_oct(*ctx->parse_point) && slex_is_oct(ctx->parse_point[1]) 
     && slex_is_oct(ctx->parse_point[2])) {
    int oct = 0;
    int fact = 64;
    for(char *it = ctx->parse_point; it < ctx->parse_point + 3; it++) {
      oct += (*it - '0') * fact;
      fact /= 8; 
    }
    ctx->parse_point += 3;
  ...
  if(*ctx->parse_point == 'x' && slex_is_hex(ctx->parse_point[1]) 
     && slex_is_hex(ctx->parse_point[2])) {
...
other_sequence:
  ...
  switch(ctx->parse_point[-1]) {
    default: ctx->parse_point--; return slex_return_err(ctx) -1;
    case 'a': return '\a';
    case 'b': return '\b';
    case 'e': return '\e';
    case 'f': return '\f';
...
static int slex_parse_char_lit(SlexContext *ctx) {
  ...
  if(*ctx->parse_point == '\\') {
    ...
    *ctx->string_store = seq;
    ...
    return slex_return_err(ctx); // empty char lit not allowed
  else {
    *ctx->string_store = *ctx->parse_point;
    ctx->parse_point++;
  }

  if(ctx->parse_point >= ctx->stream_end || *ctx->parse_point != '\'')
    return slex_return_err(ctx);
...
int slex_get_next_token(SlexContext *ctx) {
  ...
  while(ctx->parse_point <= ctx->stream_end - 1) {
